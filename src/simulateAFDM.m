function [x_daf, y_daf, H_eff, chan, sigma2, heff_info] = simulateAFDM(N, c1, c2, P, l_max, k_max, SNR_dB, QAM_order, kv)
%simulateAFDM 一帧 AFDM DAF 域仿真（支持分数多普勒截断展开）
%   N: 块大小（DAF 域子载波数），1D
...
%   输出含 sigma2 便于 Python 端使用。
    ...
    %% 2. 生成多径通道参数（含分数多普勒与 kv）
    chan = generateChannel(N, P, l_max, k_max, kv);

    %% 3. 构造 DAF 域等效信道 H_eff
    [H_eff, heff_info] = buildHeff_DAF(N, c1, c2, chan);
